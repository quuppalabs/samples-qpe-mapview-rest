...
Q.PolygonDrawable.prototype.draw = function (ctx, viewport, pass) {
	if(pass != 0)
		return false;
	...
	if(this.holes !== undefined && this.holes.length > 0) {
		for(var i = 0; i < this.holes.length; i++ ) {
			...
			for(var j = 0; j < holePoints.length; j++ ) {
				var p = viewport.transformToPixels(holePoints[holePoints.length-1-j]);  // holepoints must be added in reverse order!
				if(j === 0)
					ctx.moveTo(p[0], p[1]);
				else
					ctx.lineTo(p[0], p[1]);
