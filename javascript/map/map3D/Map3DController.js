...
//		camera = new THREE.CombinedCamera( window.innerWidth / 2, window.innerHeight / 2, 70, 1, 1000, - 500, 1000 );
	...
	this.__updateDrawable = function(drawable, tag) {
		...
		var x, y, z;
		if(pos !== undefined && pos.type === "position") {
			x = pos.x;
			y = pos.y;
			z = pos.z;
		} else {
			// Make Proximity or Presence tags vanish by overriding the coordinates to the very far corner...
		...
		drawable.position.z = z;
		drawable.shadowDrawable.position.x = x;
		drawable.shadowDrawable.position.y = y;
		...
		if(drawable.position.z < 0)
			...
			drawable.shadowDrawable.scale.x = drawable.shadowDrawable.scale.y = ballSize * heightFactor;
		}
	}
